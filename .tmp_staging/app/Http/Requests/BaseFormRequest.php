...
class BaseFormRequest extends FormRequest
{
    ...
    {
        return $this->headers->get('device-unique-id');
    ...
    {
        $device_type = strtolower($this->headers->get('device-type'));

        if (array_key_exists($device_type, ALL_DEVICE_TYPE) === true) {
            return ALL_DEVICE_TYPE[$device_type];
        }

        return ALL_DEVICE_TYPE['web'];

    }//end getDeviceType()


     /**
      * Get device Source from headers
      *
      * @return string Device type
      */
     ...
     */
    public function getAppExpiryCheck()
    {
        $device_type = (empty($this->headers->get('device-type')) === false &&
            in_array($this->headers->get('device-type'), ['android', 'ios']) === true) ? $this->headers->get('device-type') : '';
        $app_version = (empty($this->headers->get('app-version')) === false) ? $this->headers->get('app-version') : '';
