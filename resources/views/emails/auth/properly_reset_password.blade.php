...
<div class="strip"></div>
<div class="header-text"><span class="h-text-1">Properly</span><span class="h-text-2">Password</span></div></td>
</tr>
<tr>
<td style="background-color: #f3f3f3;" valign="top" align="center">
<div class="main-content" style="font-family: 'Poppins' !important;">
    <img src="{{ $view_data['lock']}}" class="image-icon" align="middle">
<h1 class="heading-1">Reset Password</h1>
 
<h2 class="heading-2">
    @if (false === empty($view_data['reset_link']))
        You tried resetting your password. Please follow following link to reset your password. This link will expire in 24 Hours.
    @elseif (false === empty($view_data['otp']))
        You tried resetting your password. Please use the following OTP to reset your password. This OTP will expire in {{ OTP_EXPIRY_TIME }} minutes.
    @endif
</h2>
    @if (false === empty($view_data['reset_link']))
        <a href="{{ $view_data['reset_link'] }}" target="_blank" rel="noopener" class="link">{{ $view_data['reset_link'] }}</a>
    @elseif (false === empty($view_data['otp']))
        {{ $view_data['otp'] }}
    @endif
